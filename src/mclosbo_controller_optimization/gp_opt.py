...
class GaussianProcessOptimization(object):
    ...
    def _add_data_point(self, gp, x, y, context=None, adaptive_scaling=False):
        ...
        if adaptive_scaling:
            # scale Y to [-1, 1]
            logger.debug("Adaptive scaling for objective is used!")
            self.y_unscaled = np.vstack([self.y_unscaled, y])
            mean = np.mean(self.y_unscaled)
            if np.max(self.y_unscaled) - np.min(self.y_unscaled) == 0:
                self.y_scaled = (self.y_unscaled - mean)
            else:
                self.y_scaled = (self.y_unscaled - mean) / (np.max(self.y_unscaled) - np.min(self.y_unscaled))

            logger.debug("scaling max: {}".format(np.max(self.y_unscaled)))
            logger.debug("scaling min: {}".format(np.min(self.y_unscaled)))
            gp.set_XY(np.vstack([gp.X, x]), self.y_scaled)
        else:
            gp.set_XY(np.vstack([gp.X, x]),
                  np.vstack([gp.Y, y]))

    # def add_new_data_point(self, x, y, context=None):
    def add_new_data_point(self, x, index_x, y, context=None):
        """
        ...
        for i, gp in enumerate(self.gps):
            ...
            if np.any(not_nan):
                ...
                if i>0:
                    ...
                    self._add_data_point(gp, x[not_nan, :], y[not_nan, [i]], adaptive_scaling=True)

        # Update global data stores
        self._x = np.concatenate((self._x, x), axis=0)
        self._y = np.concatenate((self._y, y), axis=0)
        self.update_last_point(index_x, y)
    ...
    def update_last_point(self, index_x, y):
        raise NotImplementedError("Must override method update_last_point()!")
...
class SafeOpt(GaussianProcessOptimization):
    ...
    def __init__(self, gp, parameter_set, fmin, beta_dict, lipschitz=None,
        ...
        if beta_dict["style"] == "fiedler" or beta_dict["style"] == "fiedler-lipschitz":
            logger.debug("Fiedler-bounds used!")
            B = beta_dict["B"]
            noise_variance = beta_dict["noise_variance"]
            delta = beta_dict["delta"]
            lamb = beta_dict["lambda"]  # gpr noise variance
            lamb_hat = max(1, lamb)
            R_beta = noise_variance ** 0.5
            # assuming the same kernel, B and R for all performance and safety functions we can use the same beta everywhere
            # also here, we use noise_variance instead of R ** 2 as in as in AAAI paper, since for beta
            beta = lambda t: B + \
                             (R_beta / lamb ** 0.5) * math.sqrt(
            ...
            logger.debug("Chowdhury-bounds used!")
            B = beta_dict["B"]
            R = beta_dict["R"]
            delta = beta_dict["delta"]
            noise_variance = beta_dict["noise_variance"]
            ...
            logger.debug("Srinivas-bounds used!")
            B = beta_dict["B"]
            R = beta_dict["R"]
            delta = beta_dict["delta"]
            noise_variance = beta_dict["noise_variance"]
        ...
        self.G = self.S.copy()
        self.M = self.S.copy()
        if not np.isscalar(self.index_x0):
            for i, index_x in enumerate(self.index_x0):
                 self.last_y = self.y0[i].flatten()
                 self.last_x_index = index_x
    ...
    def compute_safe_set(self):
        ...
        if self.safety == "pure-lipschitz":
            ...
            ### TODO: Include possibility of no safety constraint on performance function
            
            delta_S = np.where(
                np.all(np.logical_or(np.greater(self.last_y[:, None] - self.R[:, None] - self.liptschitz[:, None] * np.linalg.norm(
                    self.inputs[self.last_x_index] - self.inputs[next_points_indices],
                    axis=1)[None, :], self.fmin[:, None]),
                                     np.equal(self.last_y[:, None] - self.R[:, None] - self.liptschitz[:, None] * np.linalg.norm(
                                         self.inputs[self.last_x_index] - self.inputs[next_points_indices],
                                         axis=1)[None, :], self.fmin[:, None])), axis=0), 1, 0)
    ...
    def compute_sets(self, full_sets=False):
        ...
        # Update safe set
        self.compute_safe_set()
        self.S = np.logical_and(self.S, np.logical_not(self.S_x0))
    ...
    def get_new_query_point(self, ucb=False, asynchronous=False, opt=None):
        """
        ...
        if ucb:
            ...
            var = np.empty((self.inputs.shape[0], len(self.gps)),
                          dtype=np.float64)
            ## determine last optimized datapoint mean
            ## add fictional datapoint to GP
            for i in range(len(self.gps)):
                fictional_gp = self.gps[i]
                mean, _ = self.gps[i].predict_noiseless(self.inputs)    
                fictional_gp.set_XY(np.vstack([fictional_gp.X, opt]), np.vstack([fictional_gp.Y, 0]))
                _, variance = self.gps[i].predict_noiseless(self.inputs)
                var[:, i] = variance.squeeze()
                MG = np.logical_or(self.M, self.G)
                MG_indices = np.nonzero(MG)[0]
                value = np.max((variance[MG]), axis=1)
                max_id = np.argmax(value)
                x = self.inputs[MG, :][max_id, :]
        ...
        if self.num_contexts:
            return x[:-self.num_contexts], MG_indices[max_id]
        else:
            return x, MG_indices[max_id]

    def optimize(self, context=None, ucb=False, asynchronous=False, opt=None):
        ...
        return self.get_new_query_point(ucb=ucb, asynchronous=asynchronous, opt=opt)
...
class SafeOptSwarm(GaussianProcessOptimization):
    ...
    def get_new_query_point(self, swarm_type):
        ...
        if num_safe >= self.swarm_size and num_safe != len(safe):
            ...
            logger.warning("Warning: {} unsafe points removed. "
        ...
        if swarm_type != 'greedy':
            ...
            logger.debug("At the end of swarm {}, {} points were appended to"
    ...
    def optimize(self, ucb=False):
        ...
        if ucb:
            logger.debug('Using ucb criterion.')
            return x_maxi
        ...
        logger.debug("The best maximizer has std. dev. %f" % std_maxi)
        logger.debug("The best expander has std. dev. %f" % std_exp)
        logger.debug("The greedy estimate of lower bound has value %f" %
