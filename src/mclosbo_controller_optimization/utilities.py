...
def sample_function_with_fixed_B(kernel, bounds, noise_bound, num_samples, B_nom, mean_function=None):
    inputs = linearly_spaced_combinations(bounds, num_samples)
    cov = kernel.K(inputs) + np.eye(inputs.shape[0]) * 1e-6
    sigma_f = 1
    alpha_temp = np.random.normal(scale=sigma_f, size=(num_samples ** len(bounds), 1))
    B_temp = math.sqrt(np.dot(alpha_temp.T, np.dot(cov, alpha_temp)))
    alpha = (B_nom / B_temp) * alpha_temp

    def f(x, noise=True):
        x = np.atleast_2d(x)
        y = np.dot(kernel.K(x, inputs), alpha)
        if mean_function is not None:
            y += mean_function(x)
        if noise:
            # y += np.sqrt(noise_var) * np.random.randn(x.shape[0], 1)
            y += noise_bound * np.random.uniform(low=-1, high=1, size=(x.shape[0], 1))
        return y

    return f


def sample_gp_function(kernel, bounds, noise_bound, num_samples,
